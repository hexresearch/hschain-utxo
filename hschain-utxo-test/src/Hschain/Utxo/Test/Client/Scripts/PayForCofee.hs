...
payForCofeeBob :: C.ClientSpec -> IO ()
payForCofeeBob client = do
...
payForCofeeAlice :: C.ClientSpec -> IO ()
payForCofeeAlice client = do
  Scene{..} <- initUsers client
  let (User alice  aliceBox1)  = scene'alice
      (User bob    bobBox1)    = scene'bob
      (User john   johnBox1)   = scene'john
  ...
  SendResult masterBox2 johnBox3 <- debugSend
        "Master sends to john 1 coin"
        master masterBox1 john 1
  SendResult masterBox2 johnBox4 <- debugSend
...
debugSendDelayed :: Text -> Wallet -> BoxId -> Wallet -> Int -> Money -> IO SendResultDelayed
debugSendDelayed msg from fromBox to heightDiff amount = do
  T.putStrLn msg
...
sendTxDelayed :: Wallet -> BoxId -> Wallet -> Int -> Money -> IO SendResultDelayed
sendTxDelayed from fromBox to delayDiff amount = do
  ...
  currentHeight <- fmap (fromInteger . either (const 0) id) $ C.call (wallet'client from) C.getHeight
  totalAmount <- fmap (fromMaybe 0) $ getBoxBalance from fromBox
  ...
  txResp <- C.call (wallet'client from) $ C.postTx tx
  pPrint txResp
  return (SendRes backBox toBox)
